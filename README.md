...
- `MyConverteriPad`: iOS 앱. `FFmpeg-iOS` 패키지를 통해 앱 프로세스 내부에서 `ffmpeg`를 호출하며, iPad와 iPhone을 함께 지원합니다.
...
4. iOS 앱은 [`FFmpeg-iOS`](https://github.com/kewlbear/FFmpeg-iOS.git)와 [`FFmpeg-iOS-Support`](https://github.com/kewlbear/FFmpeg-iOS-Support.git)에 의존합니다. 둘 다 LGPL 계열이므로 공개 배포 전 라이선스 고지와 소스 제공 요건을 다시 확인하세요.
...
iOS 타깃은 외부 실행 파일을 실행하지 않습니다. 대신 `FFmpeg-iOS` Swift Package가 제공하는 `FFmpegSupport` 브리지를 통해 앱 내부에서 `FFmpeg_main`을 호출합니다.

- 패키지 해상도: `xcodebuild -resolvePackageDependencies -project MyConverter/MyConverter.xcodeproj -scheme MyConverteriPad`
- iPhone 빌드: `xcodebuild -project MyConverter/MyConverter.xcodeproj -scheme MyConverteriPad -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.4' build`
- iPad 빌드: `xcodebuild -project MyConverter/MyConverter.xcodeproj -scheme MyConverteriPad -destination 'platform=iOS Simulator,name=iPad Pro 11-inch (M5),OS=26.4' build`

iOS 빌드 시에는 Swift Package Manager가 FFmpeg iOS 바이너리 아티팩트를 자동으로 내려받습니다.
